...
import uk.gov.hmrc.carfstubs.models.request.RegisterWithoutIDRequest
import uk.gov.hmrc.play.bootstrap.backend.controller.BackendController

import javax.inject.Inject
import scala.concurrent.Future

class RegistrationWithoutIdStubController @Inject() (
  ...
  def register: Action[JsValue] = Action.async(parse.json) { implicit request =>
    request.body.validate[RegisterWithoutIDRequest] match {
      case JsSuccess(payload, _) =>
        ...
        val response = returnResponseWithoutId(payload)
