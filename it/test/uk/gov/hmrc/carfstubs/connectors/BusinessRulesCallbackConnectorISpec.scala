...
class BusinessRulesCallbackConnectorISpec
    extends ApplicationWithWiremock
    with Matchers
    with ScalaFutures
    with IntegrationPatience {

  lazy val connector: BusinessRulesCallbackConnector = app.injector.instanceOf[BusinessRulesCallbackConnector]
  ...
  ".callback" - {
    ...
    "must return Unit given a 200 response" in {
      ...
      val result = connector.callback(testConversationId, testDownloadUrl).value.futureValue
      result mustBe Right(())
    }
    ...
    "must return XmlValidationError given a 422 response" in {
      ...
      val result = connector.callback(testConversationId, testDownloadUrl).value.futureValue
      result mustBe Left(XmlValidationError)
    }
